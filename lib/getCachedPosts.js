var CachedPosts = require('../models/CachedPosts'),
    ...
    scraper = require('stacked-scraper');
...
module.exports = function (users, callback) {
  if (users.confirmedMembers) {
    ...
    var now = Date.now();
    var lastScrape = Date.parse(users.last.timestamp);
    ...
    getCachedPostsForUser(users, function (err, user) {
      ...
      if (scrapeAgain) {
        ...
        scraper(batch, secrets, function (err, newPosts) {
          console.log(newPosts['54c154f7dabe50b561312032'].posts)
